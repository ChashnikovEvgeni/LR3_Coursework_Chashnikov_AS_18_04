...
int main(int argc, char** argv)
{
	...
	Texture2D floor("../fl.jpg");
	
	//Texture2D tex4("../3.jpg");
	Texture2D tex5("../skybox3.jpg");
	
	Texture2D sun("../sun.jpg");
	Texture2D wood("../wood.jpg");
	Texture2D tree("../tree.jpg");
	Texture2D wing("../wing.jpg");
	Texture2D str("../str.jpg");
	Texture2D snow("../snow.jpg");
	Texture2D pugovica("../yy.jpg");
	Texture2D carrot("../carrot.jpg");
	
	Texture2D moon("../moon.jpg");
	Texture2D metal("../metal.jpg");
	Texture2D window1("../window.png");

	Texture2D shlepa("../Shlepa.png");
	Texture2D grass("../grass.jpg");
	Texture2D ships("../ships.jpg");
	Texture2D robot("../robot.jpg");
	//Texture2D listok("../1.png");
	//Создаем и загружаем геометрию поверхности
	GLuint vaoFonar1, vaoFonar2;
	GLsizei fonar1 = CreateCylinder(0.6f, 5.0f, 7, vaoFonar1);
	GLsizei fonar2 = CreateCone(2.0f, 3.0f, 7, vaoFonar2);
	...
	GLuint vaoSun;
	GLsizei Sun = CreateSphere(6.0f, 36,vaoSun);

	GLuint vaoTree, vaoTree1, vaoTree11, vaoTree2;
	GLsizei Tree = CreateCone(3.0f, 4.0f, 7, vaoTree);
	GLsizei Tree1 = CreateCone(1.0f, 2.0f, 6, vaoTree1);
	GLsizei Tree11 = CreateCone(2.0f, 2.8f, 6, vaoTree11);
	GLsizei Tree2 = CreateCub(0.5f, 2.0f, 0.5, vaoTree2);
	...
	GLsizei DragonFlyWing = CreateCub(0.15f, 3.5f, 0.5f, vaoDragonFlyWing);
	GLsizei DragonFlyWingSmall = CreateCub(0.05f, 2.0f, 0.5f, vaoDragonFlyWingSmall);


	GLuint vaoSnow1, vaoSnow2, vaoSnow3, vaoCarrot, vaoB;
	GLsizei Snowsmall = CreateSphere(0.8f, 12, vaoSnow1);
	GLsizei Snow = CreateSphere(1.0f, 12, vaoSnow2);
	GLsizei Snowbig = CreateSphere(1.2f, 12, vaoSnow3);
	GLsizei Carrot = CreateCone(0.15f, 3.0f, 5, vaoCarrot);
	GLsizei B = CreateSphere(0.1f, 12, vaoB);
	GLuint vaoPlaneta1;
	GLsizei Planeta1 = CreateSphere(2.7f, 12, vaoPlaneta1);
	GLuint vaoForest;
	GLsizei Forest = CreateCone(1.0f, 3.0f, 72, vaoForest);
	GLuint vaoShips;
	GLsizei Ships = CreateSphere(0.5f, 25, vaoShips);
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		float xtimer = sin(timer)*10;
		float ytimer = cos(timer)*5;

		// движущийся источник света
		//program.SetUniform("pointLights[0].position", make_float3(0.0f, 0.32f, 2.03f));
	/*	program.SetUniform("pointLights[0].position", make_float3(xtimer, -15.0f,ytimer* ytimer));
		...
		program.SetUniform("pointLights[0].mult", 5.4f);
		...
		*/
		...
		/*program.SetUniform("pointLights[1].position", make_float3(xtimer, -15.0f, ytimer));
		...
		program.SetUniform("pointLights[1].mult", 5.2f);*/




		//направленное освещение
		program.SetUniform("directedLights[0].direction", make_float3(5.0f, 21.0f, 0.0f));
		program.SetUniform("directedLights[0].color", make_float3(1.0f, 1.0f, 0.0f));
		program.SetUniform("directedLights[0].ambient", make_float3(0.5f, 0.5f, 0.5f));
		...
		program.SetUniform("material.alpha", 1.0f);

		program.SetUniform("typeOfTexture", false);


		//glBindVertexArray(vaoFonar1); GL_CHECK_ERRORS;
		//{
		//	bindTexture(program, 0, "material.texture_diffuse", metal);
		...
		//	model = transpose(mul(mul(translate4x4(float3(0.0f, 20.0f, 0.0f)), translate4x4(float3(xtimer, -15.0f, ytimer * ytimer))), rotate_X_4x4((90) * LiteMath::DEG_TO_RAD)));
		...
		//glBindVertexArray(vaoFonar2); GL_CHECK_ERRORS;
		//{
		//	bindTexture(program, 0, "material.texture_diffuse", metal);
		//	model = transpose(mul(translate4x4(float3(0.0f, 15.0f, 0.0f)), translate4x4(float3(xtimer, -15.0f, ytimer * ytimer))));
		//	program.SetUniform("model", model); GL_CHECK_ERRORS;
		//	glDrawElements(GL_TRIANGLE_STRIP, fonar2, GL_UNSIGNED_INT, nullptr); GL_CHECK_ERRORS;
		//}
		//glBindVertexArray(0); GL_CHECK_ERRORS;
		...
		//	bindTexture(program, 0, "material.texture_diffuse", metal);
		//	//model = transpose(mul(mul(translate4x4(float3(0.0f, 15.0f, 0.0f)), mul(rotate_Y_4x4((timer * 50) * LiteMath::DEG_TO_RAD), translate4x4(float3(15.0f, 0.0f, 0.0f)))), rotate_Z_4x4((timer * 10) * LiteMath::DEG_TO_RAD)));
		//	model = transpose(mul(mul(translate4x4(float3(0.0f, 20.0f, 0.0f)), translate4x4(float3(xtimer, -15.0f, ytimer ))), rotate_X_4x4((90) * LiteMath::DEG_TO_RAD)));
		//	program.SetUniform("model", model); GL_CHECK_ERRORS;
		//	glDrawElements(GL_TRIANGLE_STRIP, Planeta1, GL_UNSIGNED_INT, nullptr); GL_CHECK_ERRORS;
		//}
		//glBindVertexArray(0); GL_CHECK_ERRORS;
		...
		//{
		//	bindTexture(program, 0, "material.texture_diffuse", metal);
		//	model = transpose(mul(translate4x4(float3(0.0f, 15.0f, 0.0f)), translate4x4(float3(xtimer, -15.0f, ytimer ))));
		//	program.SetUniform("model", model); GL_CHECK_ERRORS;
		//	glDrawElements(GL_TRIANGLE_STRIP, fonar2, GL_UNSIGNED_INT, nullptr); GL_CHECK_ERRORS;
		//}
		//glBindVertexArray(0); GL_CHECK_ERRORS;
		...
		{
			bindTexture(program, 0, "material.texture_diffuse", grass);
			float4x4 model;
		...
		{
			bindTexture(program, 0, "material.texture_diffuse", robot);
			float4x4 timeModel = mul(rotate_Z_4x4(0.0f * LiteMath::DEG_TO_RAD), mul(rotate_Z_4x4((0) * LiteMath::DEG_TO_RAD), translate4x4(float3(10.0f, 0.0f, 0.0f))));
			model = transpose(mul(translate4x4(float3(0.0f, 20.0f+5*sin(timer), 0.0f)), rotate_Y_4x4((timer *40) * LiteMath::DEG_TO_RAD)));

			program.SetUniform("model", model); GL_CHECK_ERRORS;
			glDrawElements(GL_TRIANGLE_STRIP, Sun, GL_UNSIGNED_INT, nullptr); GL_CHECK_ERRORS;
		}
		glBindVertexArray(0); GL_CHECK_ERRORS;
		...
		glBindVertexArray(vaoTree);
		{
			...
			model = transpose(translate4x4(float3(0.0f, -13.0f, 0.0f)));
			program.SetUniform("model", model); GL_CHECK_ERRORS;
		...
		glBindVertexArray(0); GL_CHECK_ERRORS;
		glBindVertexArray(vaoTree1);
		{
			bindTexture(program, 0, "material.texture_diffuse", tree);
			model = transpose(translate4x4(float3(0.0f, -9.0f, 0.0f)));
			program.SetUniform("model", model); GL_CHECK_ERRORS;
			glDrawElements(GL_TRIANGLE_STRIP, Tree1, GL_UNSIGNED_INT, nullptr); GL_CHECK_ERRORS;


		}
		glBindVertexArray(0); GL_CHECK_ERRORS;
		glBindVertexArray(vaoTree11);
		{
			bindTexture(program, 0, "material.texture_diffuse", tree);
			model = transpose(translate4x4(float3(0.0f, -11.0f, 0.0f)));
			program.SetUniform("model", model); GL_CHECK_ERRORS;
			glDrawElements(GL_TRIANGLE_STRIP, Tree11, GL_UNSIGNED_INT, nullptr); GL_CHECK_ERRORS;


		}
		glBindVertexArray(0); GL_CHECK_ERRORS;
		glBindVertexArray(vaoTree2);
		{
			bindTexture(program, 0, "material.texture_diffuse", wood);
			model = transpose(mul(translate4x4(float3(0.0f, -15.0f, 0.0f)), rotate_Z_4x4((90) * LiteMath::DEG_TO_RAD)));
			program.SetUniform("model", model); GL_CHECK_ERRORS;
			glDrawElements(GL_TRIANGLE_STRIP, Tree2, GL_UNSIGNED_INT, nullptr); GL_CHECK_ERRORS;
		...
		{
			bindTexture(program, 0, "material.texture_diffuse", tree);
			model = transpose(translate4x4(float3(15.0f, -13.0f, -10.0f)));
			program.SetUniform("model", model); GL_CHECK_ERRORS;
			glDrawElements(GL_TRIANGLE_STRIP, Tree, GL_UNSIGNED_INT, nullptr); GL_CHECK_ERRORS;
		...
		{
			bindTexture(program, 0, "material.texture_diffuse", tree);
			model = transpose(translate4x4(float3(15.0f, -9.0f, -10.0f)));
			program.SetUniform("model", model); GL_CHECK_ERRORS;
			glDrawElements(GL_TRIANGLE_STRIP, Tree1, GL_UNSIGNED_INT, nullptr); GL_CHECK_ERRORS;
		...
		{
			bindTexture(program, 0, "material.texture_diffuse", tree);
			model = transpose(translate4x4(float3(15.0f, -11.0f, -10.0f)));
			program.SetUniform("model", model); GL_CHECK_ERRORS;
			glDrawElements(GL_TRIANGLE_STRIP, Tree11, GL_UNSIGNED_INT, nullptr); GL_CHECK_ERRORS;
		...
		{
			bindTexture(program, 0, "material.texture_diffuse", wood);
			model = transpose(mul(translate4x4(float3(15.0f, -15.0f, -10.0f)), rotate_Z_4x4((90) * LiteMath::DEG_TO_RAD)));
			program.SetUniform("model", model); GL_CHECK_ERRORS;
			glDrawElements(GL_TRIANGLE_STRIP, Tree2, GL_UNSIGNED_INT, nullptr); GL_CHECK_ERRORS;
		...
		{
			bindTexture(program, 0, "material.texture_diffuse", tree);
			model = transpose(translate4x4(float3(-25.0f, -13.0f, -10.0f)));
			program.SetUniform("model", model); GL_CHECK_ERRORS;
			glDrawElements(GL_TRIANGLE_STRIP, Tree, GL_UNSIGNED_INT, nullptr); GL_CHECK_ERRORS;
		...
		{
			bindTexture(program, 0, "material.texture_diffuse", tree);
			model = transpose(translate4x4(float3(-25.0f, -9.0f, -10.0f)));
			program.SetUniform("model", model); GL_CHECK_ERRORS;
			glDrawElements(GL_TRIANGLE_STRIP, Tree1, GL_UNSIGNED_INT, nullptr); GL_CHECK_ERRORS;
		...
		{
			bindTexture(program, 0, "material.texture_diffuse", tree);
			model = transpose(translate4x4(float3(-25.0f, -11.0f, -10.0f)));
			program.SetUniform("model", model); GL_CHECK_ERRORS;
			glDrawElements(GL_TRIANGLE_STRIP, Tree11, GL_UNSIGNED_INT, nullptr); GL_CHECK_ERRORS;
		...
		{
			bindTexture(program, 0, "material.texture_diffuse", wood);
			model = transpose(mul(translate4x4(float3(-25.0f, -15.0f, -10.0f)), rotate_Z_4x4((90) * LiteMath::DEG_TO_RAD)));
			program.SetUniform("model", model); GL_CHECK_ERRORS;
			glDrawElements(GL_TRIANGLE_STRIP, Tree2, GL_UNSIGNED_INT, nullptr); GL_CHECK_ERRORS;
		...
		//	model =transpose(mul(translate4x4(float3(0.0f, 0.0f, -timer)), mul(rotate_Y_4x4((timer*50) * LiteMath::DEG_TO_RAD), translate4x4(float3(10.0f, 0.0f, 0.0f)))));
		//	//model = transpose(translate4x4(float3(0.0f, 0.0f, -timer)));
		//	program.SetUniform("model", model); GL_CHECK_ERRORS;
		...
		//{
		//	bindTexture(program, 0, "material.texture_diffuse", wing);
		//	model = transpose(mul(mul(translate4x4(float3(0.0f, 0.0f, -timer)), mul(rotate_Y_4x4((timer * 50) * LiteMath::DEG_TO_RAD), translate4x4(float3(10.0f, 0.0f, 0.0f)))), mul(rotate_Z_4x4((90) * LiteMath::DEG_TO_RAD), rotate_Z_4x4((-timer * 250) * LiteMath::DEG_TO_RAD))));
		...
		/*glBindVertexArray(vaoSnow1);
		{
			

			bindTexture(program, 0, "material.texture_diffuse", snow);
			model = transpose(translate4x4(float3(8.0f, -10.5f, 8.0f)));
			program.SetUniform("model", model); GL_CHECK_ERRORS;
			glDrawElements(GL_TRIANGLE_STRIP, Snowsmall, GL_UNSIGNED_INT, nullptr); GL_CHECK_ERRORS;

		}
		glBindVertexArray(0); GL_CHECK_ERRORS;
		glBindVertexArray(vaoSnow2);
		{
			

			bindTexture(program, 0, "material.texture_diffuse", snow);
			model = transpose(translate4x4(float3(8.0f, -12.f, 8.0f)));
			program.SetUniform("model", model); GL_CHECK_ERRORS;
			glDrawElements(GL_TRIANGLE_STRIP, Snow, GL_UNSIGNED_INT, nullptr); GL_CHECK_ERRORS;


		}
		glBindVertexArray(0); GL_CHECK_ERRORS;
		glBindVertexArray(vaoSnow3);
		{
			

			bindTexture(program, 0, "material.texture_diffuse", snow);
			model = transpose(translate4x4(float3(8.0f, -13.5f, 8.0f)));
			program.SetUniform("model", model); GL_CHECK_ERRORS;
			glDrawElements(GL_TRIANGLE_STRIP, Snowsmall, GL_UNSIGNED_INT, nullptr); GL_CHECK_ERRORS;
		...
		/*glBindVertexArray(vaoCarrot);
		{
			bindTexture(program, 0, "material.texture_diffuse", carrot);
			model = transpose(mul(translate4x4(float3(8.0f, -10.5f, 8.0f)), (rotate_Z_4x4((-9090) * LiteMath::DEG_TO_RAD))));
			program.SetUniform("model", model); GL_CHECK_ERRORS;
			glDrawElements(GL_TRIANGLE_STRIP, DragonFlyTail, GL_UNSIGNED_INT, nullptr); GL_CHECK_ERRORS;
		}
		glBindVertexArray(0); GL_CHECK_ERRORS;*/
	...
	/*	glBindVertexArray(vaoB);
		{
			bindTexture(program, 0, "material.texture_diffuse", pugovica);
			model = transpose(translate4x4(float3(8.3f, -10.2f, 7.0f)));
			program.SetUniform("model", model); GL_CHECK_ERRORS;
			glDrawElements(GL_TRIANGLE_STRIP, B, GL_UNSIGNED_INT, nullptr); GL_CHECK_ERRORS;

			bindTexture(program, 0, "material.texture_diffuse", pugovica);
			model = transpose(translate4x4(float3(8.1f, -10.2f, 9.0f)));
			program.SetUniform("model", model); GL_CHECK_ERRORS;
			glDrawElements(GL_TRIANGLE_STRIP, B, GL_UNSIGNED_INT, nullptr); GL_CHECK_ERRORS;
		}
		glBindVertexArray(0); GL_CHECK_ERRORS;
...
*/
		...
		/*glBindVertexArray(vaoSnow1);
		{
			...
			bindTexture(program, 0, "material.texture_diffuse", snow);
			model = transpose(translate4x4(float3(-8.0f, -10.5f, 13.0f)));
			program.SetUniform("model", model); GL_CHECK_ERRORS;
			glDrawElements(GL_TRIANGLE_STRIP, Snowsmall, GL_UNSIGNED_INT, nullptr); GL_CHECK_ERRORS;
		...
		glBindVertexArray(vaoSnow2);
		{
			...
			bindTexture(program, 0, "material.texture_diffuse", snow);
			model = transpose(translate4x4(float3(-8.0f, -12.f, 13.0f)));
			program.SetUniform("model", model); GL_CHECK_ERRORS;
			glDrawElements(GL_TRIANGLE_STRIP, Snow, GL_UNSIGNED_INT, nullptr); GL_CHECK_ERRORS;
		...
		glBindVertexArray(vaoSnow3);
		{
			...
			bindTexture(program, 0, "material.texture_diffuse", snow);
			model = transpose(translate4x4(float3(-8.0f, -13.5f, 13.0f)));
			program.SetUniform("model", model); GL_CHECK_ERRORS;
			glDrawElements(GL_TRIANGLE_STRIP, Snowsmall, GL_UNSIGNED_INT, nullptr); GL_CHECK_ERRORS;
		...
		{
			bindTexture(program, 0, "material.texture_diffuse", carrot);
			model = transpose(mul(translate4x4(float3(-8.0f, -10.5f, 13.0f)), (rotate_Z_4x4((-9090) * LiteMath::DEG_TO_RAD))));
			program.SetUniform("model", model); GL_CHECK_ERRORS;
			glDrawElements(GL_TRIANGLE_STRIP, DragonFlyTail, GL_UNSIGNED_INT, nullptr); GL_CHECK_ERRORS;
		}
		glBindVertexArray(0); GL_CHECK_ERRORS;*/
	...
	/*	glBindVertexArray(vaoB);
		{
			bindTexture(program, 0, "material.texture_diffuse", pugovica);
			model = transpose(translate4x4(float3(-8.3f, -10.2f, 12.0f)));
			program.SetUniform("model", model); GL_CHECK_ERRORS;
			glDrawElements(GL_TRIANGLE_STRIP, B, GL_UNSIGNED_INT, nullptr); GL_CHECK_ERRORS;

			bindTexture(program, 0, "material.texture_diffuse", pugovica);
			model = transpose(translate4x4(float3(-8.1f, -10.2f, 14.0f)));
			program.SetUniform("model", model); GL_CHECK_ERRORS;
			glDrawElements(GL_TRIANGLE_STRIP, B, GL_UNSIGNED_INT, nullptr); GL_CHECK_ERRORS;
		}
		glBindVertexArray(0); GL_CHECK_ERRORS;
*/
		...
		{
			bindTexture(program, 0, "material.texture_diffuse", wood);
			model = transpose(mul(translate4x4(float3(5.0f, -15.0f, -8.0f)), rotate_Z_4x4((90) * LiteMath::DEG_TO_RAD)));
			program.SetUniform("model", model); GL_CHECK_ERRORS;
			glDrawElements(GL_TRIANGLE_STRIP, Tree2, GL_UNSIGNED_INT, nullptr); GL_CHECK_ERRORS;
		...
		{
			bindTexture(program, 0, "material.texture_diffuse", wood);
			model = transpose(mul(translate4x4(float3(-5.0f, -15.0f, -20.0f)), rotate_Z_4x4((90) * LiteMath::DEG_TO_RAD)));
			program.SetUniform("model", model); GL_CHECK_ERRORS;
			glDrawElements(GL_TRIANGLE_STRIP, Tree2, GL_UNSIGNED_INT, nullptr); GL_CHECK_ERRORS;
		...
		{
			bindTexture(program, 0, "material.texture_diffuse", wood);
			model = transpose(mul(translate4x4(float3(-31.0f, -15.0f, 10.0f)), rotate_Z_4x4((90) * LiteMath::DEG_TO_RAD)));
			program.SetUniform("model", model); GL_CHECK_ERRORS;
			glDrawElements(GL_TRIANGLE_STRIP, Tree2, GL_UNSIGNED_INT, nullptr); GL_CHECK_ERRORS;
		...
		program.SetUniform("material.alpha", 1.0f);
		glBindVertexArray(vaoShips);
		{

			float4x4 modelShips;
			bindTexture(program, 0, "material.texture_diffuse", ships);
			for (int i = 1; i < 3; i++)
			{
				
		

				modelShips = transpose(mul(translate4x4(float3((2.0f + i + 20*sin(timer/2)), -12.0f, -5.0f + i * 10+20*cos(timer/2))), scale4x4(float3(2.0f, 2.0f, 2.0f))));
				program.SetUniform("model", modelShips); GL_CHECK_ERRORS;
				glDrawElements(GL_TRIANGLE_STRIP, Ships, GL_UNSIGNED_INT, nullptr); GL_CHECK_ERRORS;


				modelShips = transpose(mul(translate4x4(float3((2.0f + i + 20*sin(timer/2)), -12.0f, -5.0f + i * 10+20*cos(timer/2))), scale4x4(float3(5.0f, 0.1f, 5.0f))));
				program.SetUniform("model", modelShips); GL_CHECK_ERRORS;
				glDrawElements(GL_TRIANGLE_STRIP, Ships, GL_UNSIGNED_INT, nullptr); GL_CHECK_ERRORS;

			}
			program.SetUniform("pointLights[1].position", make_float3(2.0f + 1 + 20*sin(timer/2), -12.0f, -5.0f + 1 * 10+20*cos(timer/2)));
			program.SetUniform("pointLights[1].color", make_float3(1.0f, 0.0f, 0.0f));
			program.SetUniform("pointLights[1].ambient", make_float3(5.6f, 5.6f, 5.6f));
			program.SetUniform("pointLights[1].specular", make_float3(1.0f, 1.0f, 1.0f));
			program.SetUniform("pointLights[1].mult", 1.2f);

			program.SetUniform("pointLights[0].position", make_float3(2.0f + 2 + 20*sin(timer/2), -12.0f, -5.0f + 2 * 10+20*cos(timer/2)));
			program.SetUniform("pointLights[0].color", make_float3(1.0f, 0.0f, 0.0f));
			program.SetUniform("pointLights[0].ambient", make_float3(5.6f, 5.6f, 5.6f));
			program.SetUniform("pointLights[0].specular", make_float3(1.0f, 1.0f, 1.0f));
			program.SetUniform("pointLights[0].mult", 1.2f);
		...
		glBindVertexArray(vaoForest);
		{
			...
			{
				bindTexture(program, 0, "material.texture_diffuse", tree);
				float4x4 model1 = transpose(mul(translate4x4(float3(sin(20 * i) * 30.0f, -15.0f, cos(i) * 30.0f)), scale4x4(float3(0.5f, (0.4 + sin(i / 2) / 3 * pow((-1), i)), 0.5f))));
		...
		program.SetUniform("material.alpha", 0.3f);
		glBindVertexArray(vaoPlaneta1);
		{
			bindTexture(program, 0, "material.texture_diffuse", window1);
			model = transpose(mul(mul(translate4x4(float3(0.0f, 15.0f, 0.0f)), mul(rotate_Y_4x4((50) * LiteMath::DEG_TO_RAD), translate4x4(float3(15.0f, 0.0f, 0.0f)))), rotate_Z_4x4((10) * LiteMath::DEG_TO_RAD)));
			program.SetUniform("model", model); GL_CHECK_ERRORS;
			glDrawElements(GL_TRIANGLE_STRIP, Planeta1, GL_UNSIGNED_INT, nullptr); GL_CHECK_ERRORS;
		}
		glBindVertexArray(0); GL_CHECK_ERRORS;
		...
		//наращиваем поворот каждый кадр как скорость вращения умноженная на время, прошедшее за кадр
		d_alpha += 250.0f * deltaTime;
		d_tetha += 10.0f * deltaTime;
	}
