...
void main()
{
	...
	result_color = vec4(0.0f, 0.0f, 0.0f, material.alpha);
	if (!typeOfTexture) {
           	for (int i = 0; i < POINT_LIGHTS_NUM; ++i)
		         result_color += vec4(lightContrib(pointLights[i], material, vNormal, vFragPosition, viewDir), 0.0f);

            for (int i = 0; i < DIRECTED_LIGHTS_NUM; ++i)
		         result_color += vec4(lightContribDirected(directedLights[i], vNormal, vFragPosition, viewDir), 0.0f);	
			...
			for (int i = 0; i < POINT_LIGHTS_NUM; ++i)
	    	result_color += vec4(lightContribNorm(pointLights[i], material, normal, vFragPosition, viewDir, tangentFragPos), 0.0f);

            for (int i = 0; i < DIRECTED_LIGHTS_NUM; ++i)
	    	result_color += vec4(lightContribDirected(directedLights[i], normal, vFragPosition, viewDir), 0.0f);
